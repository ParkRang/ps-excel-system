...
3) 엑셀 대용량 데이터 처리
   10만 건의 데이터 처리 및 엑셀 생성을 위해 SXSSFWorkbook을 사용하였습니다. 100개의 행을 메모리에 유지하고 나머지는 디스크에 기록하는 스트리밍 방식을 이용, 메모리 사용량을 줄이는 방식을 선택할 수 있었습니다. 일정 수의 행만 메모리에 유지하므로, 이번 과제의 리소스 제한 환경에 적합하다고 판단하여 선택하였습니다.
4) 동시 요청
   ThreadPoolTaskExecutor를 사용, 여러 사용자의 요청을 동시에 처리할 수 있도록 구성하였습니다. ThreadPool을 이용하여, 요청을 한번에 처리할 수 있도록 하고, 조건에 맞게 작업을 관리할 수 있다는 점이 장점이라 생각하여 선택하였습니다. Thread를 적용하여 동시 작업 수를 제한하고, 사용량을 점검하였습니다. CPU 0.5, Memory 1GB라는 제한 조건을 고려하여 관리하도록 설계하였습니다.
5) Transational
...
6) 실시간 폴링
   상태 갱신 방식으로 페이지 새로고침, 폴링, SSE를 비교하였습니다. 페이지 새로고침은 사용성이 좋지 않았고, SSE는 실시간성이 우수하지만 현재 요구사항 규모에서는 구현 복잡도가 증가한다고 판단했습니다. 따라서 2초 간격 폴링을 적용하여 구현 복잡도와 요구사항 충족 사이의 균형을 선택하였습니다.
...
현재는 Order 데이터를 모두 조회한 뒤 엑셀을 생성합니다.

데이터 규모가 더욱 증가하는 환경에서는 ExcelJob을 조회하며 
JDBC Streaming 또는 Cursor 기반 조회를 적용하여
조회 단계의 메모리 사용량을 추가로 줄일 수 있습니다.

또한 현재 상태 조회는 Polling 방식으로 구현하였지만,
실시간성이 중요한 환경에서는 SSE를 적용하여
불필요한 요청을 줄일 수 있습니다.
